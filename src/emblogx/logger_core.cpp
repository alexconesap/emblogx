...
namespace emblogx {
    ...
    void log_va(uint8_t target, Level lvl, const char* module, const char* fmt, va_list args) {
        ...
        int header_len = std::snprintf(line, sizeof(line), "%s[%s][%s] ", EMBLOGX_LOG_PREFIX,
                                       level_str, mod_str);
        if (header_len < 0) {
            return;
        }
        ...
        // Forward via va_copy. Strictly the current call sites only consume the
        // va_list once so passing `args` directly would also work, but va_copy
        // is the textbook-correct pattern for any function that takes a forwarded
        // va_list — it makes the implementation safe against future code paths
        // (level filters, retries, fallback formatting) that might want to walk
        // the args a second time.
        va_list args_copy;
        va_copy(args_copy, args);
        int body_len = std::vsnprintf(
                line + header_len, sizeof(line) - static_cast<size_t>(header_len), fmt, args_copy);
        va_end(args_copy);
        if (body_len < 0) {
            return;
        }
